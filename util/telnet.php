...
*/
	class Telnet
	{
		...
		public function connect() 
		{
			...
			if (!$this -> socket)
				throw new Exception("Can't connect to {$this -> host} on port {$this -> port}");
		...
		public function disconnect()
		{
			if ($this -> socket)
			{
				if (!fclose($this -> socket))
					throw new Exception("Failed closing telnet-socket");
		...
		protected function readTo($promptstr)
		{
			...
			do 
			{
				if (time() > $until_t) //crash if timeout exceeded
					throw new Exception("Couldn't find the requested prompt string '$promptstr' within {$this -> timeout} seconds");
				...
				if ($char === FALSE)
				{
					...
					throw new Exception("Couldn't find the requested: '{$promptstr}', it was not in the data returned from server: {$this -> buffer}");
				}
				if ($char == $this -> IAC)
				{
		...
		protected function handleTelnetOptions()
		{
			...
			if ($c != $this -> IAC)
			{
				...
				{
					throw new Exception('Error: Unknown control char ' . ord($c));
				}
			} 
			else 
			{
